			...
			<div class="submit-address dashboard-list">
                <form method="GET">
                    ...
                    <div class="row pad-20">
                        <div class="col-lg-12">
                           	<?php foreach ($fetch_customer_orders as $order_item): ?>
		                       	<div class="comment">
		                            <div class="comment-author">
		                                <a href="#">
		                                    <img src="<?= base_url(); ?><?php echo $order_item->item_image1 ?>" alt="comments-user" style="width:110px; height:100px;">
		                                </a>
		                            </div>
		                            <div class="comment-content">
		                                <div class="comment-meta">
		                                   <h5><?php echo $order_item->first_name ?> <?php echo $order_item->last_name ?> <a style="margin-left:10px; font-size:12px;color:rgb(96, 89, 238);" href="<?= base_url('profile') ?>/<?php echo $order_item->oauth_uid; ?>">View Profile</a></h5>
		                                    <div class="comment-meta">
		                                    	<?php 
		                                    		$req_date=explode(' ', $order_item->request_date);
		                                    	?>
		                                       Request date &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <?php echo date('h:i A', strtotime($order_item->request_date)) ?> || <?php echo date("d M Y", strtotime($req_date[0])); ?><br>
		                                       Borrowing date : <?php echo date("d M Y  ",$order_item->created_at); ?> 
		                                    </div>
		                                </div>
		                                <ul>
		                                    <li>Item :<span> <?php echo $order_item->item_name ?></span></li>
		                                    <?php if($order_item->booking_name=='day'){ ?>
		                                    	<li>Start date : <span> <?php echo date("d M Y", strtotime($order_item->start_date)); ?></span></li>
		                                    	<li>End date : <span> <?php echo date("d M Y", strtotime($order_item->end_date)); ?></span></li>
		                                    <?php } ?>

		                                    <?php if($order_item->booking_name=='hourly'){ ?>
		                                    	<li>Pickup date : <span> <?php echo date("d M Y", strtotime($order_item->pickup_date)); ?></span></li>
		                                    	<li>Pickup time : <span> <?php echo $order_item->pick_up ?></span></li>
		                                    	<li>Total Book : <span> <?php echo $order_item->total_hours ?> hrs</span></li>
		                                    <?php } ?>

		                                    <?php if($order_item->booking_name=='monthly'){ ?>
		                                    	<li>Total Month : <span> <?php echo $order_item->total_month ?> months</span></li>
		                                    <?php } ?>
		                                    
		                                    <li>User quote :<span> <?php echo $order_item->user_comments ?></span></li>
		                                    <li>Total Pay :<span> <?php echo ($order_item->payment_amount)-30 ?></span></li>
			    ...
			    <form method="post" action="<?= base_url('Item-Cancel') ?>">
			      ...
			      <div class="modal-content">
			        ...
			        <div class="modal-body">
			          	<span id="output"></span>
			          	<div class="_1v8t1fb row" style="font-weight:bold;font-size:20px;color:#6059EE;">Enter Cancel reason</div>
